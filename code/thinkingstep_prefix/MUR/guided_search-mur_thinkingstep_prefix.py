...
def generate_vanilla_step(policy_model, input_text, step_idx):
    """Generate a single step using vanilla CoT"""
    ...
    return step_text, logp, len(output.token_ids)
    ...
    for idx, example in enumerate(test_data):
        ...
        for step_idx in range(args.max_steps):
            try:
                ...
                # First, always generate a vanilla step to get cur_signal
                vanilla_step, vanilla_logp, vanilla_tokens = generate_vanilla_step(
                    policy_model, input_text, step_idx)
                all_policy_output_tokens += vanilla_tokens
                cur_signal = vanilla_logp
                
                # Check if we should do test-time scaling
                should_scale = (args.thinking_step_prefix_length > 0 and 
                               step_idx > 0 and 
                               np.exp(cur_signal) < np.exp(momentum_uncertainty) * args.scaling_rate)
                
                if should_scale:
                    # Generate prefix candidates instead of using vanilla
                    ...
                    candidate_traj.append({
                        ...
                        'used_test_time_scaling': 'True',
                        'vanilla_step': vanilla_step
                    })
                else:
                    # Use the vanilla step
                    current_traj.append(vanilla_step)
                    
                    candidate_traj.append({
                        'step_idx': str(step_idx),
                        'step_uncertainty': str(np.exp(-cur_signal)),
                        'momentum_uncertainty/gamma': str(np.exp(-momentum_uncertainty) / args.momentum_rate),
                        'used_test_time_scaling': 'False',
                        'vanilla_step': vanilla_step
                    })
